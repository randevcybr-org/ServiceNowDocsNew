...
![Integration run status chart example from host detection](../../../vulnerability-response/image/qualysv_10-intr-runschart.png "Sample Qualys Integration Run Status chart")
...
-   Qualys Vulnerable Item Ingestion Performance Throughput: Compare daily vulnerable item ingestion throughput for the Qualys Host Detection integration. Throughput is measured in items per hour. This report shows the overall performance on a particular day and not at the individual parameter level. ![Qualys integration run status](../../../secops-integration-vr/qualys/image/Qualys_integration_run_status_v10.3.png)
