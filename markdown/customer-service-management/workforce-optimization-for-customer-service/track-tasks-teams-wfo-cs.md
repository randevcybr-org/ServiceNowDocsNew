...
breadcrumb: [Assigning work, Use, Workforce Optimization for Customer Service, Customer Service Management]
---
...
2.  Select the **Work Scheduler** icon.
    ...
    -   In the contextual side panel, select the Filter icon \( ![Filter icon](../image/filter-icon.png)\) and then select any filter, add the desired criteria, and select **Apply** to narrow down the display of the team members in the calendar. You can filter based on your team's assignment group, skills, location, schedule plan, or shift plan.
3.  If you don’t have any queue configurations in the Work scheduler, select the queue settings icon \(![Queue Settings icon](../image/queuefilter.png)\) and then enable up to five work item types.
...
6.  Assign a work item to an agent.
    ...
    1.  Select a task that you want to assign to an agent.
        ...
        To add your own matching rule, see [Create a criteria for a matching rule in Work scheduler](assignment-rules-work-scheduler-wfo-cs.md).
    ...
    3.  Click and drag your mouse device for the time span on the calendar for which you want the agent to work on the task.
        ...
        **Note:** From the work queue, when you select a task that has been assigned to an agent, the application auto-scrolls to the page that has the time span for the task and displays the schedule for the agent.
