...
description: Sets common file attributes, such as timestamps, size, permissions, and UID/GID, for a file or directory on an SFTP server.
locale: en-US
release: australia
...
Name or IP address of the SFTP server that contains the file you wish to remove.
...
Path of the directory that contains the files you wish to remove.
...
File or directory permissions to set for the user and group specified. This value must be specified in Octal notation only. For example, `755`.**Note:** The permissions number is an internal value returned by the Get File List command.
...
Override the timestamp when the file or directory was last modified.**Note:**

-   Timestamp must be in epoch format.
-   Access and modification timestamps must be set together as a pair.
...
Override the timestamp when the file or directory was last accessed.**Note:**
