...
-   As an Admin, enable the property to work on budgeting. For more information, see [Enable financial budget allocation for planning items in Strategic Planning](enable-fin-budget-spw.md).
-   As an Admin, configure the attribute to allocate and approve budget by cost type or expense type. For more information, see [Configure budget attribute at instance-level to allocate budget](config-budget-allocation-attribute-spw.md).
...
4.  Enable the **Budget allocation** toggle \(![Budget allocation toggle enabled to view budget.](../images/fin-budget-allocation-toggle.png)\).
...
5.  You can manage the budget by one of the following ways for the selected time scale at monthly, quarterly, or yearly level.
    ...
    **Note:** Unapproved budget values are indicated with ![Tick mark in a circle representing the unapproved budget](../images/fin-copy-budget-icon.png) icon.

6.  Select **Approve budget** \(![Approve budget button.](../images/fin-approve-budget-icon.png)\).
...
7.  On the confirmation window, select **Approve** \(![Approve button.](../images/fin-approve-icon.png)\).
