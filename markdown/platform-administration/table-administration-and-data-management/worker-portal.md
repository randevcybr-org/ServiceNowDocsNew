...
-   Displays the notifications for the submitted, rejected, and recalled time sheets under the show notifications icon \(![Show notifications icon](../image/NotificationIcon.png)\). Each entry displays the number of time cards and their state. Click on a notification item to open the time sheet.
