...
# Create a Workvivo external content connector
...
A source system administrator must have already configured your Workvivo source system to allow access by the Workvivo external content connector. For details on configuring these settings in the source system, see [Configure Workvivo for external content indexing](configure-workvivo-external-content-indexing.md).
...
4.  On the Choose source page, select the **Workvivo** tile, then select **Next**.
...
Unique name for this Workvivo external content connector.
...
Base URL for the Workvivo API.If you don't have the API base URL, ask your Workvivo administrator for it.
...
URL for your Workvivo instance host. This is typically `https://<organization-name>.workvivo.us`, where `<organization-name>` is the name of your organization.If you don't have the instance base URL, ask your Workvivo administrator for it.
...
Workvivo ID for your organization. This is typically a numeric value, such as `1755225`.If you don't have the organization ID, ask your Workvivo administrator for it.
...
Token for an API key defined in the app that grants the connector access to your Workvivo instance's content.If you don't have the API key token for the app, ask your Workvivo administrator for it.
...
8.  On the Crawl settings page, modify any default crawl settings that you want to override for this connector, then select **Next**.
    ...
    If you want to skip this step for now, select **Skip** instead of **Next**. You can modify the crawl settings for this connector from the External Content Admin Home page. For details on this procedure and the available crawl settings, see [Configure crawl settings for a Workvivo external content connector](configure-crawl-settings-workvivo-external-content-connector.md).
...
-   To learn how to configure your new connector's crawl settings, see [Configure crawl settings for a Workvivo external content connector](configure-crawl-settings-workvivo-external-content-connector.md).
