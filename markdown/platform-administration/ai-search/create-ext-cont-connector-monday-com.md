---
title: Create a monday.com external content connector
description: Create an external content connector to retrieve searchable content and security principals from your monday.com source system.
locale: en-US
release: australia
product: AI Search
...
# Create a monday.com external content connector
...
A source system administrator must have already configured your monday.com source system to allow access by the monday.com external content connector. For details on configuring these settings in the source system, see [Configure monday.com for external content indexing](configure-monday-com-external-content-indexing.md).
...
4.  On the Choose source page, select the **monday.com** tile, then select **Next**.
...
Unique name for this monday.com external content connector.
...
URL for the monday.com API. This URL typically has the value `https://api.monday.com/v2`.If you don't have the monday.com API URL, ask your monday.com administrator for it.
...
Personal API token for a monday.com user account with either administrator privileges or API access. The monday.com external content connector uses this token to authenticate as the admin user when running crawls.If you don't have the authentication token, ask your monday.com administrator for it.
...
8.  On the Crawl settings page, modify any default crawl settings that you want to override for this connector, then select **Next**.
    ...
    If you want to skip this step for now, select **Skip** instead of **Next**. You can modify the crawl settings for this connector from the External Content Admin Home page. For details on this procedure and the available crawl settings, see [Configure crawl settings for a monday.com external content connector](configure-crawl-settings-monday-com-external-content-connector.md).
...
-   To learn how to configure your new connector's crawl settings, see [Configure crawl settings for a monday.com external content connector](configure-crawl-settings-monday-com-external-content-connector.md).
