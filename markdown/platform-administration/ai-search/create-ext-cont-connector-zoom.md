---
title: Create a Zoom external content connector
description: Create an external content connector to retrieve searchable content and security principals from your Zoom source system.
locale: en-US
release: australia
product: AI Search
...
# Create a Zoom external content connector

Create an external content connector to retrieve searchable content and security principals from your Zoom source system.

## Before you begin

A source system administrator must have already configured your Zoom source system to allow access by the Zoom external content connector. For details on configuring these settings in the source system, see [Configure Zoom for external content indexing](configure-zoom-external-content-indexing.md).
...
4.  On the Choose source page, select the **Zoom** tile, then select **Next**.
...
Unique name for this Zoom external content connector.
...
Account ID for the Server-to-Server OAuth app that grants the connector access to your Zoom content. If you don't have the account ID, ask your Zoom administrator for it.
...
Client ID for the Server-to-Server OAuth app that grants the connector access to your Zoom content. If you don't have the client ID, ask your Zoom administrator for it.
...
Client secret for the Server-to-Server OAuth app that grants the connector access to your Zoom content. If you don't have the client secret, ask your Zoom administrator for it.
...
8.  On the Crawl settings page, modify any default crawl settings that you want to override for this connector, then select **Next**.
    ...
    If you want to skip this step for now, select **Skip** instead of **Next**. You can modify the crawl settings for this connector from the External Content Admin Home page. For details on this procedure and the available crawl settings, see [Configure crawl settings for a Zoom external content connector](configure-crawl-settings-zoom-external-content-connector.md).
...
-   To learn how to configure your new connector's crawl settings, see [Configure crawl settings for a Zoom external content connector](configure-crawl-settings-zoom-external-content-connector.md).
