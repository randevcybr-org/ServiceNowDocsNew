---
title: Supported keys and functions
description: The Send keys component in the RPA Desktop Design Studio supports multiple keys, special keys, and functions. The keyboard keys must be enclosed within \[\]. For example, \[Back\].
locale: en-US
release: australia
topic_type: reference
last_updated: "2026-03-12"
reading_time_minutes: 4
breadcrumb: [Actions \(UI\), Automation components, RPA Desktop Design Studio, Workflow Data Fabric]
---

# Supported keys and functions

The Send keys component in the RPA Desktop Design Studio supports multiple keys, special keys, and functions. The keyboard keys must be enclosed within \[\]. For example, \[Back\].

|Supported Keyboard keys|Description|
|-----------------------|-----------|
|A|A key|
|AbntC1|ABNT\_C2 \(Brazilian\) key.|
|Add|Add key.|
|Apps|Application key \(Microsoft Natural Keyboard\).|
|Attn|ATTN key.|
|B|B key.|
|Back|Backspace key.|
|BrowserBack|Browser Back key.|
|BrowserFavorites|Browser Favorites key.|
|BrowserForward|Browser Forward key.|
|BrowserHome|Browser Home key.|
|BrowserRefresh|Browser Refresh key.|
|BrowserSearch|Browser Search key.|
|BrowserStop|Browser Stop key.|
|C|C key.|
|Cancel|Cancel key.|
|Capital|Caps Lock key.|
|CapsLock|Caps Lock key.|
|Clear|Clear key.|
|CrSel|CRSEL key.|
|D|D key.|
|D0|0 \(zero\) key.|
|D1|1 \(one\) key|
|D2|2 key.|
|D3|3 key.|
|D4|4 key.|
|D5|5 key.|
|D6|6 key.|
|D7|7 key.|
|D8|8 key.|
|D9|9 key.|
|DbeAlphanumeric|DBE\_ALPHANUMERIC key.|
|DbeCodeInput|DBE\_CODEINPUT key.|
|DbeDbcsChar|DBE\_DBCSCHAR key.|
|DbeDetermineString|DBE\_DETERMINESTRING key.|
|DbeEnterDialogConversionMode|DBE\_ENTERDLGCONVERSIONMODE key.|
|DbeEnterImeConfigureMode|DBE\_ENTERIMECONFIGMODE key.|
|DbeEnterWordRegisterMode|DBE\_ENTERWORDREGISTERMODE key.|
|DbeFlushString|DBE\_FLUSHSTRING key.|
|DbeHiragana|DBE\_HIRAGANA key.|
|DbeKatakana|DBE\_KATAKANA key.|
|DbeNoCodeInput|DBE\_NOCODEINPUT key.|
|DbeNoRoman|DBE\_NOROMAN key.|
|DbeRoman|DBE\_ROMAN key.|
|DbeSbcsChar|DBE\_SBCSCHAR key.|
|DeadCharProcessed|Key that is used with another key to create a single combined character.|
|Decimal|Decimal key.|
|Delete|Delete key.|
|Divide|Divide key.|
|Down|Down Arrow key.|
|E|E key.|
|End|End key.|
|Enter|Enter key.|
|EraseEof|ERASE EOF key.|
|Escape|ESC key.|
|Execute|Execute key.|
|ExSel|EXSEL key.|
|F|F key.|
|F1|F1 key.|
|F11|F11 key.|
|F12|F12 key.|
|F13|F13 key.|
|F14|F14 key.|
|F15|F16 key.|
|F16|F16 key.|
|F17|F17 key.|
|F18|F18 key.|
|F19|F19 key.|
|F20|F20 key.|
|F21|F21 key.|
|F22|F22 key.|
|F23|F23 key.|
|F24|F24 key.|
|F3|F3 key.|
|F4|F4 key.|
|F5|F5 key.|
|F6|F6 key.|
|F7|F7 key.|
|F8|F8 key.|
|F9|F9 key.|
|FinalMode|IME Final mode key.|
|G|G key.|
|H|H key.|
|HangulMode|IME Hangul mode key.|
|HanjaMode|IME Hanja mode key.|
|Help|Help key.|
|Home|Home key.|
|I|I key.|
|ImeAccept|IME Accept key.|
|ImeConvert|IME Convert key.|
|ImeModeChange|IME Mode change request.|
|ImeNonConvert|IME NonConvert key.|
|ImeProcessed|Special key that masks the real key that is being processed by an IME.|
|Insert|Insert key.|
|J|J key.|
|JunjaMode|IME Junja mode key.|
|K|K key.|
|KanaMode|IME Kana mode key.|
|KanjiMode|IME Kanji mode key.|
|L|L key.|
|LaunchApplication1|Launch Application1 key.|
|LaunchApplication2|Launch Application2 key.|
|LaunchMail|Launch Mail key.|
|Left|Left Arrow key.|
|LeftAlt|Left ALT key.|
|LeftCtrl|Left CTRL key.|
|LeftShift|Left Shift key.|
|LineFeed|Linefeed key.|
|LWin|Left Windows logo key \(Microsoft Natural Keyboard\).|
|M|M key.|
|MediaNextTrack|Media Next Track key.|
|MediaPlayPause|Media Play Pause key.|
|MediaPreviousTrack|Media Previous Track key.|
|MediaStop|Media Stop key.|
|Multiply|Multiply key.|
|N|N key.|
|Next|Page Down key.|
|NoName|Constant reserved for future use.|
|None|No key pressed.|
|NumLock|Num Lock key.|
|NumPad0|0 key on the numeric keypad.|
|NumPad1|1 key on the numeric keypad.|
|NumPad2|2 key on the numeric keypad.|
|NumPad3|3 key on the numeric keypad.|
|NumPad4|4 key on the numeric keypad.|
|NumPad5|5 key on the numeric keypad.|
|NumPad6|6 key on the numeric keypad.|
|NumPad7|7 key on the numeric keypad.|
|NumPad8|8 key on the numeric keypad.|
|NumPad9|9 key on the numeric keypad.|
|O|O key.|
|Oem1|OEM 1 key.|
|Oem102|OEM 102 key.|
|Oem2|OEM 2 key.|
|Oem3|OEM 3 key.|
|Oem4|OEM 4 key.|
|Oem5|OEM 5 key.|
|Oem6|OEM 6 key.|
|Oem7|OEM 7 key.|
|Oem8|OEM 8 key.|
|OemAttn|OEM ATTN key.|
|OemAuto|OEM AUTO key.|
|OemBackslash|OEM Backslash key.|
|OemBackTab|OEM BACKTAB key.|
|OemClear|OEM Clear key.|
|OemCloseBrackets|OEM Close Brackets key.|
|OemComma|OEM Comma key.|
|OemCopy|OEM COPY key.|
|OemEnlw|OEM ENLW key.|
|OemFinish|OEM FINISH key.|
|OemMinus|OEM Minus key.|
|OemOpenBrackets|OEM Open Brackets key.|
|OemPeriod|OEM Period key.|
|OemPipe|OEM Pipe key.|
|OemPlus|OEM Addition key.|
|OemQuestion|OEM Question key.|
|OemQuotes|OEM Quotes key.|
|OemSemicolon|OEM Semicolon key.|
|OemTilde|OEM Tilde key.|
|P|P key.|
|Pa1|PA1 key.|
|PageDown|Page Down key.|
|PageUp|Page Up key.|
|Pause|Pause key.|
|Play|PLAY key.|
|Print|Print key.|
|PrintScreen|Print Screen key.|
|Prior|Page Up key.|
|Q|Q key.|
|R|R key.|
|Return|Return key.|
|Right|Right Arrow key.|
|RightAlt|Right ALT key.|
|RightCtrl|Right CTRL key.|
|RightShift|Right Shift key.|
|RWin|Right Windows logo key \(Microsoft Natural Keyboard\).|
|S|S key.|
|Scroll|Scroll Lock key.|
|Select|Select key.|
|SelectMedia|Select Media key.|
|Separator|Separator key.|
|Sleep|Computer Sleep key.|
|Snapshot|Print Screen key.|
|Space|Spacebar key.|
|Subtract|Subtract key.|
|System|Special key that masks the real key that is being processed as a system key.|
|T|T key.|
|Tab|Tab key.|
|U|U key.|
|Up|Up Arrow key.|
|V|V key.|
|VolumeDown|Volume Down key.|
|VolumeMute|Volume Mute key.|
|VolumeUp|Volume Up key.|
|W|W key.|
|X|X key.|
|Y|Y key.|
|Z|Z key.|
|ZOOM|Zoom key.|

**Parent Topic:**[Actions \(UI\)](../concept/actions-ui.md)

