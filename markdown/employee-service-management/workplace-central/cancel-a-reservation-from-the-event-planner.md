---
title: Cancel a reservation from the Event planner
description: As a reservation planner, the Event planner enables you to cancel a reservation at any time.
locale: en-US
release: australia
product: Workplace Central
classification: workplace-central
topic_type: task
last_updated: "2026-03-25"
reading_time_minutes: 1
breadcrumb: [Working with Event planner, Use, Workplace Central, Workplace Service Delivery, Employee Service Management]
---

# Cancel a reservation from the Event planner

As a reservation planner, the Event planner enables you to cancel a reservation at any time.

## Before you begin

Ensure that you have installed the Calendar component \(sn\_calendar\) of version 23.2.0 or later.

Role required: sn\_wsd\_rsv.reservation\_planner

## Procedure

1.  Navigate to **All** &gt; **Workplace Central** &gt; **Workplace Central**.

    You can also open Workplace Central from the Employee Center directly. Navigate to **Workspaces** &gt; **Workplace Central**.

    The Workplace Analytics dashboard opens.

2.  Select the **Event planner** icon from the menu \(![Event planner icon.](../images/event-planner-icon.png)\).

    The Event Planner dashboard opens.

3.  To cancel any current day reservation, do the following:

    1.  Go to the **Reservations** metrics section.

    2.  Select one of the following tiles to view as a list:

        -   **Planned reservations for today**
        -   **Reservations with extra services for today**
4.  To view the active reservations, do one of the following:

    -   To view all the reservations, go to the **All Reservations** section.
    -   To view the reservation created by you or for you, go to the **My reservations** section.
    To view more reservations, select **View all**.

5.  Select the reservation that you want to cancel.

    The reservation opens in a new tab. The details of the reservation are displayed.

6.  Select **Cancel reservation** to cancel the reservation.

7.  On the confirmation window, select **Confirm cancellation**.


## Result

The reservation is canceled. The cancellation notes are automatically generated.

**Parent Topic:**[Working with Event planner](../concept/working-with-event-planner.md)

