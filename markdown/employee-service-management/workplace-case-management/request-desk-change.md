...
3.  Select **Request a desk change**.

4.  On the form, fill in the fields.

<table id="table_pzr_4zb_qqb"><thead><tr><th>
...
Name of the employee for whom you are raising the move request. If you are raising the request for yourself, select your name.

</td></tr><tr><td>

**When is the requested move date?**
...
Find a space on a floor

</td><td>

Option to directly select the current space on the floor map.
...
Items to be moved from the current desk to the new target desk.
...
Attached files that support the move request if required.
...
The move request is submitted.

If the state of the move request is Closed complete, it means that the move request has been completed successfully. If the state is set to Closed incomplete, it means that the move request could not be completed due to various reasons. You can raise another request if required.
