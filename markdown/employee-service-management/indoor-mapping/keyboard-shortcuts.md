...
4.  To display the keyboard shortcuts for **Directions** click the help icon \(![Help or question mark icon](../images/help_question_mark_icon.png)\) available on the top right pane of the Map Studio.
    ...
    -   Stop drawing: Press **esc** \(Mac\) or **Esc** \(Windows\).
    -   Delete selected items: When a node or route is selected, press **fn + delete** \(Mac\) or **Backspace** \(Windows\).
5.  To display list of shortcuts for Places:

    -   Create a new place with a point: **cmd + i** \(Mac\) or **ctrl + i**.
    -   Create a new polygon: **cmd +p** \(Mac\) or **ctrl +p**
    -   Edit a selected place: **cmd +e** \(Mac\) or **ctrl +e** \(Windows\)
    -   Save the selected place: **cmd +s** \(Mac\) or **ctrl +s** \(Windows\)
